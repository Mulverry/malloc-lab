...
int mm_init(void)
{
	...
	next_fit_ptr = heap_listp + (2*WSIZE);
	heap_listp += (2*WSIZE); 
...
void *find_next_fit(size_t asize) {
    ...
    while (GET_SIZE(HDRP(bp)) > 0) { // traverse the free list
        if (!GET_ALLOC(HDRP(bp)) && (asize <= GET_SIZE(HDRP(bp)))) { // check if block is free and large enough
            last_bp = bp; // update most recently allocated block
            return bp;
        }
