...
#define SIZE_T_SIZE (ALIGN(sizeof(size_t)))  // size에서 size로 이동
...
#define GET_SIZE(p) (GET(p) & ~0x7)  /*크기 리턴. 헤더 정보에 있는 사이즈의 크기만 가져옴. (0~7은 )*/
...
#define FTRP(bp) ((char *)(bp) + GET_SIZE(HDRP(bp)) - DSIZE)    //블록의 푸터가 가리키는 포인터 리턴. bp + 블록사이즈 - DSIZE

/* Given block ptr bp, compute address of next and previous blocks */
#define NEXT_BLKP(bp) ((char *)(bp) + GET_SIZE(((char *)(bp) - WSIZE))) //다음 블록의 bp 리턴. 현재 bp + 현재블록 크기(현재 블록 헤더에 나온 블록사이즈)
...
static char *heap_listp; // p824 힙의 첫번째 포인터
static char *free_listp;

/* 
 * mm_init - initialize the malloc package. mm_malloc, mm_free를 호출하기 전에 초기화
    // 초기 힙 생성
...
int mm_init(void)
{
	/* Create the initial empty heap */
	 // 처음에 mem_sbrk(6*WSIZE) 를 호출함으로써 heap_listp 의 시작 주소를 변경시켜 준다.
	if ((heap_listp = mem_sbrk(6*WSIZE)) == (void *)-1)
		return -1;

	PUT(heap_listp, 0); //Alignment padding. heap:0에 free 넣음.
	PUT(heap_listp + (1*WSIZE), PACK(DSIZE*2, 1)); //Prologue header. heap:1(heap_listp + Wsize가 가리키는 워드)에 Dsize*2와 allocated 저장
	PUT(heap_listp + (2*WSIZE), 0); //Predessor. heap:2에 free 넣음.
	PUT(heap_listp + (3*WSIZE), 0); //Successor. heap:3에 free 넣음.
	PUT(heap_listp + (4*WSIZE), PACK(DSIZE*2, 1)); //Prologue footer.
	PUT(heap_listp + (5*WSIZE), PACK(0, 1)); //Epilogue header. heap:5에 allocated넣음.

	free_listp = heap_listp + (2*WSIZE); //첫 freelist의 bp지정: predecessor 위치.
...
/* 
 * mm_malloc - Allocate a block by incrementing the brk pointer.요청 받은 메모리 사이즈를 brk포인터를 증가시킴으로써 인접한 alignment(8)의 배수로 올려 할당한다.
 *     Always allocate a block whose size is a multiple of the alignment. 만약 맞는 크기의 가용 블록이 없다면 추가 힙 메모리를 확장 & 할당한다.
 */
void *mm_malloc(size_t size) // malloc(사이즈 byte). 사이즈만큼 메모리 할당.
{
	size_t asize; // 할당된 블록 사이즈 조정
	...
	if (size <= DSIZE) // 만약 입력받은 사이즈가 DSIZE보다 작을지라도 최소 블록SIZE(2*DSIZE)로 생성
        asize = 2*DSIZE; /
	else // // 8바이트 넘는 요청 : 오버헤드(블록 내에서 payload를 제외한 모든 것=헤더,푸터) 바이트 내에 더해주고, 가까운 8의 배수로 반올림
...
{	void *bp;
    ...
    for (bp = free_listp; GET_ALLOC(HDRP(bp)) != 1; bp = SUCC(bp)){ // freelist 처음부터 블록단위로 free블록을 만날 때까지 bp를 succ로 바꿔가며 for문 탐색
        if ((GET_SIZE(HDRP(bp))) >= asize){//할당된 사이즈보다 블록크기가 크면 
            return bp;
        }
    }
    return NULL;
...
static void *coalesce(void *bp){
	size_t prev_alloc = GET_ALLOC(FTRP(PREV_BLKP(bp))); //Previous block 할당여부
	size_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(bp))); //Next block 할당여부
	size_t size = GET_SIZE(HDRP(bp)); // 현재 블록사이즈
...
static void place(void *bp, size_t asize){
    ...
    if ((csize - asize) >= (2*DSIZE)){ //현재블록에 할당된 사이즈 넣고도 나머지가 최소블럭크기 이상으로 남으면
		//헤더와 풋터에 요청받은 용량만큼 블록배치(1은 할당된 블록)
        PUT(HDRP(bp), PACK(asize, 1));
        PUT(FTRP(bp), PACK(asize, 1));
		bp = NEXT_BLKP(bp);

		//이제 bp는 다음 블록포인터임. 다음블럭포인터에 나머지 사이즈와 free 할당함.
        PUT(HDRP(bp), PACK(csize-asize, 0)); 
        PUT(FTRP(bp), PACK(csize-asize, 0));
		add_free_block(bp);
    }
	else { //나머지가 최소블록크기보다 적게 남으면 그냥 현재블록에 모두 할당.
