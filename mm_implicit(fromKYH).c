...
#define PACK(size, alloc)   ((size) | (alloc))      //  힙에 할당할 메모리 크기와 alloc bit 을 합친다.

#define GET(p)              (*(unsigned int *)(p))              //  *p 값을 부른다.
#define PUT(p, val)         (*(unsigned int *)(p) = (val))      //  *p 에 val 을 넣는다.
...
static void *extend_heap(size_t words)
{
    ...
    //  나중에 Next 의 Header 에 입력된 Size 가 0 이면 할당을 하도록 한다.
    //  allocate bit 가 1 이어도 한다.
...
static void *next_fit(size_t asize)
{
    ...
    for (bp = saved_bp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp)) {
        if (!GET_ALLOC(HDRP(bp)) && (asize <= GET_SIZE(HDRP(bp)))) {
            saved_bp = bp;
            return bp;
        }
