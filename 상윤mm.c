...
#define GET(p) (*(unsigned int *)(p)) // 주소 p에서 unsigned int형을 가리키고 있는 것을 가져온다
#define PUT(p, val) (*(unsigned int *)(p) = (val)) // 주소 p의 unsigned int형을 가리키고 있는 곳에 value값을 넣어준다
...
#define FTRP(bp) ((char *)(bp) + GET_SIZE(HDRP(bp)) - DSIZE) // bp에서 block size를 더하고 나서 Double Word Size를 빼준다.(왼쪽으로 이동)

/* block pointer(bp)를 인자로 받아 이후, 이전 블록의 주소 리턴 */
#define NEXT_BLKP(bp) ((char *)(bp) + GET_SIZE(((char *)(bp) - WSIZE))) // (char*)(bp) + GET_SIZE(지금 블록의 Header값)=>결국 GET_SIZE가 block size인셈
#define PREV_BLKP(bp) ((char *)(bp) - GET_SIZE(((char *)(bp) - DSIZE))) // (char*)(bp) - GET_SIZE(이전 블록의 Footer값)=>결국 GET_SIZE가 block size인셈
...
void *mm_malloc(size_t size)   // 메모리 할당 -----------------------------------------------
{
    ...
    {   // adjusted size(asize) : 조정된 사이즈 2 * 16 = 32 
        asize = 2 * DSIZE;
    }
    ...
    if ((bp = extend_heap(extendsize/WSIZE)) == NULL)  // 단위 변환을 위해서 워드 사이즈로 바꿔주기 위해서 나눈다. 넘긴 사이즈만큼의 heap을 할당 받는다.
        return NULL;
    place(bp, asize); // 늘여져있는 상태에다가 bp주소에 asize를 배치
    return bp;
}
...
static void place(void *bp, size_t asize) // free block에 넣어주는 함수 ---------------------------
{
    ...
    {   
        ...
        PUT(FTRP(bp), PACK(csize-asize, 0)); // csize-asize = 나머지
    }
    else {                                // 2*DSIZE(최소 가용 블록 크기)보다 적게 남으면 모두 할당
 ...
 */
void mm_free(void *bp) //bp는 할당 되어있는  block을 가리킨다.--------------------------------------------
{
